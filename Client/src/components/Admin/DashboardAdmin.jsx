...
import {
  ...
  Sparkles, Globe, Star, ArrowUpRight,
} from "lucide-react";

/* ─────────────────────────────────────────
   FONT INJECTION — Inter (identique SuiviEvaluation)
───────────────────────────────────────── */
const FONT_STYLE = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .kpi-card:hover {
    ...
    box-shadow: 0 20px 50px rgba(13,27,94,0.17) !important;
  }
  ...
  .quick-link:hover {
    ...
    box-shadow: 0 8px 24px rgba(13,27,94,0.12) !important;
  }
  ...
  .dash-tab:hover {
    background: rgba(26,59,212,0.06) !important;
  }
  ...
  .anim-in {
    animation: fadeSlideUp 0.42s cubic-bezier(.22,1,.36,1) both;
  }
  .anim-in-1 { animation-delay: 0.04s; }
  .anim-in-2 { animation-delay: 0.08s; }
  .anim-in-3 { animation-delay: 0.12s; }
  .anim-in-4 { animation-delay: 0.16s; }
  .anim-in-5 { animation-delay: 0.20s; }
...
const resolveGroup = (role) => {
  const DG = ["Directeur Général","Directeur Général Adjoint","Chef de Division","DG","CD","DR","SUPERADMIN"];
  const CS = ["Chef de Section"];
  const CA = ["Chef d'Antenne","CC"];
  const F  = ["Formateur","FORMATEUR"];
  const CO = ["Conseiller"];
  const E  = ["Entreprise"];
  if (DG.includes(role)) return "DG";
  if (CS.includes(role)) return "CS";
  if (CA.includes(role)) return "CA";
  if (F.includes(role))  return "F";
  if (CO.includes(role)) return "C";
  if (E.includes(role))  return "E";
  return "DG";
};
...
const buildNavCategories = (role) => {
  ...
  const all = [
    { section:"Formations",        color:C.steel,   icon:BookOpen,     groups:ALL,
      items:[
    ...
    { section:"Apprenants",        color:C.navy,    icon:GraduationCap,groups:ALL_OP,
      items:[
    ...
    { section:"Présences & Notes", color:C.success, icon:ClipboardList,groups:["CA","F"],
      items:[
    ...
    { section:"Suivi complet",     color:C.success, icon:ClipboardList,groups:DG_CS,
      items:[
    ...
    { section:"Fin de session",    color:C.accent,  icon:CheckCircle2, groups:["CA","F"],
      items:[
    ...
    { section:"Entreprises",       color:C.navy,    icon:Briefcase,    groups:["DG","CS","E"],
      items:[
    ...
    { section:"Rapports",          color:C.purple,  icon:BarChart3,    groups:["DG","CS","CA","C"],
      items:[
    ...
    { section:"Centres & Équipes", color:C.purple,  icon:Building2,    groups:["DG","CS"],
      items:[
    ...
    { section:"Administration",    color:C.textSub, icon:Settings,     groups:["DG"],
      items:[
  ...
  return all
    ...
    .map(cat => ({
      ...
      items: cat.items
        ...
        .filter((item,idx,arr) => arr.findIndex(x=>x.path===item.path&&x.label===item.label)===idx),
    }))
...
const getStoredUser = () => {
  try {
    ...
    if (token) {
      ...
      return { username:p.username||p.email||"Utilisateur", role:p.role||"Directeur Général",
               niveau:p.niveau??0, region:p.region||null, centre:p.centre||null,
...
const StatCard = ({ label, value, delta, icon: Icon, color, delay=0 }) => {
  ...
  return (
    <div className={`kpi-card anim-in anim-in-${delay}`} style={{
      ...
      boxShadow:"0 2px 20px rgba(13,27,94,0.06)",
...
const SectionHeader = ({ icon: Icon, title, color, count }) => (
  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
    ...
    <div style={{ flex:1 }}>
      <p style={{ fontSize:13, fontWeight:800, color:C.navy, fontFamily:"'Inter',sans-serif", letterSpacing:"-0.2px" }}>{title}</p>
    </div>
    {count && (
      <span style={{ fontSize:10, fontWeight:700, color, background:`${color}14`, border:`1px solid ${color}30`, borderRadius:20, padding:"2px 9px", fontFamily:"'Inter',sans-serif" }}>
        {count}
      </span>
    )}
  </div>
...
const SectionBox = ({ icon, title, color, children, count, className="" }) => (
  <div className={`anim-in ${className}`} style={{
    background:"#fff", borderRadius:18, padding:"18px 18px",
    boxShadow:"0 2px 16px rgba(13,27,94,0.05)",
    border:"1.5px solid #EEF2FF",
  }}>
...
const QuickLink = ({ path, label, icon: Icon, color }) => (
  <Link to={path||"#"} style={{ textDecoration:"none", flex:"1 1 calc(50% - 4px)", minWidth:0 }}>
    <div className="quick-link" style={{
      ...
      borderRadius:12, background:"#FAFBFF", border:"1.5px solid #EEF2FF",
      cursor:"pointer", boxShadow:"0 1px 4px rgba(13,27,94,0.05)",
    }}>
...
const ActivityItem = ({ text, detail, time, dot, isLast }) => (
  <div style={{
    ...
    borderBottom: isLast ? "none" : "1px solid #F4F6FF",
    alignItems:"flex-start",
  }}>
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, paddingTop:4, flexShrink:0 }}>
      <div style={{ width:8, height:8, borderRadius:"50%", background:dot, flexShrink:0, boxShadow:`0 0 0 3px ${dot}20` }}/>
  ...
  return (
    <>
      ...
      <div className="dash-root" style={{
        minHeight:"100vh",
        background:`linear-gradient(160deg,#EDF1FC 0%,#F5F7FF 60%,#F8F6FF 100%)`,
        ...
        <div className="anim-in" style={{
          ...
          borderRadius:24, marginBottom:28, color:"#fff",
          padding:"32px 36px",
          boxShadow:`0 12px 48px rgba(13,27,94,0.32), 0 1px 0 rgba(255,255,255,0.06) inset`,
          position:"relative", overflow:"hidden",
        }}>
          {/* Decorative circles */}
          <div style={{ position:"absolute", top:-70, right:-50, width:280, height:280, borderRadius:"50%", background:"rgba(255,255,255,0.04)", pointerEvents:"none" }}/>
          <div style={{ position:"absolute", bottom:-80, right:80,  width:200, height:200, borderRadius:"50%", background:"rgba(255,255,255,0.03)", pointerEvents:"none" }}/>
          <div style={{ position:"absolute", top:20,  right:160, width:60, height:60, borderRadius:"50%", background:"rgba(245,168,0,0.15)", pointerEvents:"none" }}/>
          ...
          <div className="dash-hero-inner" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:20, flexWrap:"wrap" }}>
            {/* Left */}
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
                ...
                <div>
                  <p style={{ fontSize:10, opacity:0.55, letterSpacing:2.5, textTransform:"uppercase", fontWeight:600, fontFamily:"'Inter',sans-serif" }}>
                    Plateforme ONFPP · Guinée
                  </p>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                <span style={{
                  ...
                  borderRadius:20, padding:"5px 14px", fontSize:11.5, fontWeight:700,
                ...
                {storedUser.region && (
                  <span style={{ display:"inline-flex", alignItems:"center", gap:4, fontSize:11, opacity:0.75, fontFamily:"'Inter',sans-serif" }}>
                    <MapPin size={10}/>{storedUser.region}
                  </span>
                )}
                {storedUser.centre && (
                  <span style={{ display:"inline-flex", alignItems:"center", gap:4, fontSize:11, opacity:0.75, fontFamily:"'Inter',sans-serif" }}>
                    <Building2 size={10}/>{storedUser.centre}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="dash-hero-right" style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:12 }}>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <span style={{
                  ...
                  <Globe size={11} opacity={0.7}/>{totalModules} modules
                </span>
                <div ref={notifRef} style={{ position:"relative" }}>
                  ...
                  {notifOpen && (
                    <div style={{
                      ...
                      boxShadow:"0 12px 50px rgba(13,27,94,0.20)",
                      border:"1.5px solid #EEF2FF", zIndex:999, overflow:"hidden",
                    }}>
                      ...
                      {notifs.map((n,i)=>(
                        <div key={i} style={{ padding:"12px 18px", borderBottom:i<notifs.length-1?"1px solid #F5F7FF":"none", background:n.read?"#fff":"#F5F8FF", display:"flex", gap:10, alignItems:"flex-start" }}>
                          ...
                          <div style={{ flex:1 }}>
                            <p style={{ fontSize:12, fontWeight:600, color:C.navy, fontFamily:"'Inter',sans-serif" }}>{n.text}</p>
                            <p style={{ fontSize:11, color:C.textMuted, marginTop:1, fontFamily:"'Inter',sans-serif" }}>{n.detail} · {n.time}</p>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="dash-grid-main" style={{ display:"grid", gridTemplateColumns:"1fr 290px", gap:20, alignItems:"start" }}>
              ...
              {sections.map((sec,si)=>(
                <SectionBox key={si} icon={sec.icon} title={sec.section} color={sec.color} count={`${sec.items.length}`} className={`anim-in-${Math.min(si+1,5)}`}>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:7 }}>
                    {sec.items.map((m,mi)=>(
                      <ModuleCard key={mi} label={m.label} icon={m.icon} path={m.path} color={sec.color}/>
                    ))}
                  </div>
                </SectionBox>
              ))}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div className="anim-in anim-in-1" style={{
                background:`linear-gradient(145deg,${C.navy},#1E2E80)`,
                borderRadius:20, padding:"22px 20px",
                boxShadow:`0 8px 30px ${C.navy}30`,
                position:"relative", overflow:"hidden",
              }}>
                {/* gold top accent */}
              ...
              {/* Accès rapides */}
              <div className="anim-in anim-in-2" style={{ background:"#fff", borderRadius:18, padding:"18px 18px", border:"1.5px solid #EEF2FF", boxShadow:"0 2px 14px rgba(13,27,94,0.05)" }}>
              ...
              <div className="anim-in anim-in-3" style={{
                ...
                <p style={{ fontSize:11, color:C.textSub, lineHeight:1.6, fontFamily:"'Inter',sans-serif" }}>
        ...
        {activeTab==="activite" && (
          <div style={{ maxWidth:720 }}>
            <SectionBox icon={Activity} title="Activité récente" color={C.blue} className="anim-in">
              {activity.map((act,i)=>(
                <ActivityItem key={i} {...act} isLast={i===activity.length-1}/>
              ))}
            </SectionBox>
          </div>
        )}
