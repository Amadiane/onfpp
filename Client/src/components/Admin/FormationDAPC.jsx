...
import {
  ...
  MapPin, Building2, Clock, CheckCircle2,
  BarChart3, TrendingUp, UserCheck,
  Download, FileSpreadsheet, FileText as FilePdf,
} from "lucide-react";

/* ═══════════════════════════════════════════════════════════════════
   PALETTE — cohérente DAP (violet/purple + palette ONFPP)
═══════════════════════════════════════════════════════════════════ */
const C = {
  page: "#F8F9FD", surface: "#FFFFFF", surfaceEl: "#F5F3FF",
  navy: "#06102A", navyMid: "#0C1D5F", blue: "#1635C8",
  ...
  dap: "#6B21A8", dapMid: "#7C3AED", dapLight: "#A855F7",
  dapPale: "#F5F3FF", dapSoft: "#EDE9FE",
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  .dap { font-family: 'Outfit', sans-serif; -webkit-font-smoothing: antialiased; }
  .dap-serif { font-family: 'Fraunces', serif !important; }

  .dap-page::before {
    content: ''; position: fixed; inset: 0; pointer-events: none; z-index: 0;
    background-image: radial-gradient(circle at 1px 1px, rgba(107,33,168,.045) 1px, transparent 0);
  ...
  .dap-input:focus { border-color: ${C.dap}; box-shadow: 0 0 0 3px rgba(107,33,168,.09); }
  .dap-input::placeholder { color: ${C.textMuted}; }
  select.dap-input { cursor: pointer; }

  .dap-btn-pri {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 10px 22px; border-radius: 11px; border: none; cursor: pointer;
    font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 700;
    background: linear-gradient(135deg, ${C.navy}, ${C.dap}); color: #fff;
    box-shadow: 0 4px 18px rgba(107,33,168,.28);
    transition: all .18s cubic-bezier(.34,1.2,.64,1);
  }
  .dap-btn-pri:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(107,33,168,.38); }
  ...
  .dap-btn-sec:hover { background: ${C.surfaceEl}; border-color: ${C.iceBlue}; }

  .dap-btn-export {
    display: inline-flex; align-items: center; gap: 7px;
    padding: 9px 16px; border-radius: 10px; cursor: pointer;
    font-family: 'Outfit', sans-serif; font-size: 12px; font-weight: 700;
    border: 1.5px solid; transition: all .14s ease;
  }
  .dap-btn-excel {
    background: #F0FFF4; color: #166534; border-color: #BBF7D0;
  }
  .dap-btn-excel:hover { background: #DCFCE7; border-color: #86EFAC; }
  .dap-btn-pdf {
    background: #FFF1F2; color: #9F1239; border-color: #FECDD3;
  }
  .dap-btn-pdf:hover { background: #FFE4E6; border-color: #FDA4AF; }

  .dap-tr { transition: background .12s; cursor: pointer; }
  .dap-tr:hover { background: #F5F3FF !important; }
  ...
  .dap-pg:hover { background: ${C.surfaceEl}; border-color: ${C.iceBlue}; }
  .dap-pg.active { background: ${C.dap}; color: #fff; border-color: ${C.dap}; }
  .dap-pg:disabled { opacity: .4; cursor: not-allowed; }
  ...
  .dap-card:hover {
    ...
    box-shadow: 0 22px 52px rgba(107,33,168,.13);
    border-color: #DDD6FE;
  }

  .dap-modal { animation: dapModal .26s cubic-bezier(.22,1,.36,1) both; }
  @keyframes dapModal {
  ...
  .dap-spin { animation: dapSpin .75s linear infinite; display: inline-block; }

  @keyframes dapUp {
    from { opacity: 0; transform: translateY(20px) scale(.984); }
    to   { opacity: 1; transform: translateY(0)    scale(1); }
  }
  .dap-in { animation: dapUp .46s cubic-bezier(.22,1,.36,1) both; }
  .dap-d0 { animation-delay: .00s; } .dap-d1 { animation-delay: .07s; }
  .dap-d2 { animation-delay: .14s; } .dap-d3 { animation-delay: .21s; }
  .dap-d4 { animation-delay: .28s; }

  @keyframes dapBar { from { width: 0; } to { width: var(--bw); } }
  .dap-bar { animation: dapBar 1s cubic-bezier(.22,1,.36,1) both; }

  .dap-detail-row {
    display: flex; align-items: flex-start; gap: 10px;
    padding: 11px 0; border-bottom: 1px solid ${C.divider};
  }
  .dap-detail-row:last-child { border-bottom: none; }
  .dap-detail-label {
    font-size: 10.5px; font-weight: 800; color: ${C.textMuted};
    text-transform: uppercase; letter-spacing: .07em;
    min-width: 155px; flex-shrink: 0; padding-top: 2px;
  }
  .dap-detail-val { font-size: 13px; color: ${C.textPri}; font-weight: 500; line-height: 1.5; }
  ...
  .dap-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 16px; }

  @keyframes dapAurora {
    0%, 100% { opacity: .5; transform: scale(1); }
    50%       { opacity: .7; transform: scale(1.06) translateX(12px); }
  }
  .dap-aurora { animation: dapAurora 10s ease-in-out infinite; }

  @media (max-width: 900px) {
    .dap-grid { grid-template-columns: 1fr 1fr !important; }
    .dap-hide-sm { display: none !important; }
  }
  @media (max-width: 600px) {
    .dap-grid { grid-template-columns: 1fr !important; }
    .dap { padding: 80px 14px 52px !important; }
  }

  /* ── Print styles pour PDF ── */
  @media print {
    body * { visibility: hidden; }
    #dap-print-area, #dap-print-area * { visibility: visible; }
    #dap-print-area {
...
const buildDapId = (f, idx) => {
  const year = f.date_debut ? new Date(f.date_debut).getFullYear() : new Date().getFullYear();
  return `ONFPP-DAP-${ANTENNE_CODE(f.antenne)}-${year}-${String((idx || 0) + 1).padStart(3, "0")}`;
};
...
const fmtDExport = (d) => d ? new Date(d).toLocaleDateString("fr-FR") : "—";
...
const getSessionStatus = (debut, fin) => {
  ...
  if (!f || now <= f) return { l: "En cours",   c: C.dap,    bg: C.dapPale,   icon: TrendingUp   };
...
const StatutBadge = ({ statut }) => {
  ...
  return (
    <span className="dap-badge" style={{ background: s.bg, color: s.c, border: `1px solid ${s.c}30` }}>
      {s.l}
    </span>
  );
};
...
const SessionBadge = ({ debut, fin }) => {
  ...
  const SI = s.icon;
  return (
    <span className="dap-badge" style={{ background: s.bg, color: s.c, border: `1px solid ${s.c}25` }}>
      <SI size={9} /> {s.l}
    </span>
  );
};
...
const exportExcel = (formations, filtered) => {
  const data = filtered.map((f, i) => ({
    "ID Formation":    buildDapId(f, formations.findIndex((x) => x.id === f.id)),
    "Intitulé":        f.nom_formation || "—",
    "Organisme":       f.organisme_formation || "—",
    "Formateur":       f.nom_formateur || "—",
    "Antenne":         f.antenne_display || ANTENNE_LABEL(f.antenne),
    "Entreprise":      f.entreprise_formation || "—",
    "Date début":      fmtDExport(f.date_debut),
    "Date fin":        fmtDExport(f.date_fin),
    "Durée (jours)":   dureeJours(f.date_debut, f.date_fin) || "—",
    "Statut session":  getSessionStatus(f.date_debut, f.date_fin).l,
    "Apprenants":      f.nb_candidats || 0,
    "Créé par":        f.created_by_nom || "—",
  }));
  ...
  ws["!cols"] = [
    { wch: 28 }, { wch: 40 }, { wch: 30 }, { wch: 25 }, { wch: 14 },
    { wch: 28 }, { wch: 13 }, { wch: 13 }, { wch: 14 }, { wch: 15 },
    { wch: 12 }, { wch: 22 },
  ];

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Formations DAP");

  /* Feuille stats */
  const statsData = [
    ["Statistiques — Formations DAP", ""],
    ["Total formations", filtered.length],
    ["Planifiées",       filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "Planifiée").length],
    ["En cours",         filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "En cours").length],
    ["Terminées",        filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "Terminée").length],
    ["Total apprenants", filtered.reduce((a, f) => a + (f.nb_candidats || 0), 0)],
  ...
  XLSX.writeFile(wb, `ONFPP_DAP_Formations_${new Date().toISOString().slice(0,10)}.xlsx`);
};

/** Export PDF via impression navigateur (print stylé) */
const exportPDF = (formations, filtered) => {
  const rows = filtered.map((f, _i) => {
    const idx = formations.findIndex((x) => x.id === f.id);
    const fid = buildDapId(f, idx);
    const ss  = getSessionStatus(f.date_debut, f.date_fin);
    const dur = dureeJours(f.date_debut, f.date_fin);
    return `
      <tr style="border-bottom:1px solid #E8EDFC;">
        <td style="padding:7px 10px;font-family:monospace;font-size:10px;color:#6B21A8;font-weight:800;">${fid}</td>
        <td style="padding:7px 10px;font-size:11px;font-weight:700;color:#06102A;max-width:200px;">${f.nom_formation || "—"}</td>
        <td style="padding:7px 10px;font-size:10.5px;color:#3A4F8C;">${f.antenne_display || ANTENNE_LABEL(f.antenne)}</td>
        <td style="padding:7px 10px;font-size:10.5px;color:#3A4F8C;">${f.organisme_formation || "—"}</td>
        <td style="padding:7px 10px;font-size:10.5px;color:#3A4F8C;">${f.nom_formateur || "—"}</td>
        <td style="padding:7px 10px;font-size:10.5px;color:#3A4F8C;">${fmtD(f.date_debut)} → ${fmtD(f.date_fin)}</td>
        <td style="padding:7px 10px;font-size:10px;color:#D4920A;font-weight:700;">${dur ? dur + "j" : "—"}</td>
        <td style="padding:7px 10px;">
          <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;
            background:${ss.bg};color:${ss.c};border:1px solid ${ss.c}30;">${ss.l}</span>
        </td>
        <td style="padding:7px 10px;font-size:12px;font-weight:800;color:#047A5A;text-align:center;">${f.nb_candidats || 0}</td>
      </tr>`;
  }).join("");

  const totalApprenants = filtered.reduce((a, f) => a + (f.nb_candidats || 0), 0);
  const planif  = filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "Planifiée").length;
  const enCours = filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "En cours").length;
  const terminees = filtered.filter((f) => getSessionStatus(f.date_debut, f.date_fin).l === "Terminée").length;

  const html = `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8"/>
<title>Formations DAP — ONFPP</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;600;700;800&display=swap');
  * { box-sizing: border-box; margin:0; padding:0; }
  body { font-family:'Outfit',sans-serif; background:#fff; color:#06102A; padding:30px 36px; }
  h1 { font-size:22px; font-weight:800; color:#06102A; margin-bottom:4px; }
  .subtitle { font-size:12px; color:#8497C8; margin-bottom:20px; }
  .tri { height:4px; display:flex; border-radius:2px; overflow:hidden; margin-bottom:18px; width:80px; }
  .tri div { flex:1; }
  .kpis { display:grid; grid-template-columns:repeat(5,1fr); gap:10px; margin-bottom:22px; }
  .kpi { background:#F5F3FF; border-radius:10px; padding:12px 14px; border:1px solid #EDE9FE; text-align:center; }
  .kpi-val { font-size:22px; font-weight:800; }
  .kpi-lbl { font-size:10px; color:#8497C8; margin-top:3px; }
  table { width:100%; border-collapse:collapse; font-size:11px; }
  thead tr { background:#F5F3FF; border-bottom:2px solid #EDE9FE; }
  th { padding:9px 10px; text-align:left; font-size:9.5px; font-weight:800; color:#8497C8; text-transform:uppercase; letter-spacing:.08em; }
  tbody tr:nth-child(even) { background:#FAFAF9; }
  .footer { margin-top:18px; font-size:10px; color:#8497C8; border-top:1px solid #E8EDFC; padding-top:10px; display:flex; justify-content:space-between; }
  .badge-dap { display:inline-flex; align-items:center; gap:5px; background:#F5F3FF; color:#6B21A8; border:1px solid #6B21A830; border-radius:20px; padding:2px 9px; font-size:10px; font-weight:800; }
</style>
</head>
<body>
  <div class="tri"><div style="background:#E02020"/><div style="background:#D4920A"/><div style="background:#047A5A"/></div>
  <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:6px;">
    <div>
      <span class="badge-dap">DAP — Direction Apprentissage Professionnel</span>
      <h1 style="margin-top:8px;">Catalogue Formations Apprentissage</h1>
      <p class="subtitle">ONFPP Guinée — Export du ${new Date().toLocaleDateString("fr-FR", {day:"2-digit",month:"long",year:"numeric"})}</p>
    </div>
  </div>

  <div class="kpis">
    <div class="kpi"><div class="kpi-val" style="color:#6B21A8;">${filtered.length}</div><div class="kpi-lbl">Formations</div></div>
    <div class="kpi"><div class="kpi-val" style="color:#D4920A;">${planif}</div><div class="kpi-lbl">Planifiées</div></div>
    <div class="kpi"><div class="kpi-val" style="color:#6B21A8;">${enCours}</div><div class="kpi-lbl">En cours</div></div>
    <div class="kpi"><div class="kpi-val" style="color:#047A5A;">${terminees}</div><div class="kpi-lbl">Terminées</div></div>
    <div class="kpi"><div class="kpi-val" style="color:#047A5A;">${totalApprenants}</div><div class="kpi-lbl">Apprenants</div></div>
  </div>
  ...
  <div class="footer">
    <span>ONFPP — Office National de Formation Professionnelle et Promotion — Guinée</span>
    <span>DAP · ${filtered.length} formation${filtered.length > 1 ? "s" : ""} · ${totalApprenants} apprenant${totalApprenants > 1 ? "s" : ""}</span>
  </div>
...
/* ═══════════════════════════════════════════════════════════════════
   MODAL DÉTAIL FORMATION DAP
═══════════════════════════════════════════════════════════════════ */
const DAPDetailModal = ({ formation, dapId, onClose }) => {
  ...
  const InfoRow = ({ label, value, node }) => (
    <div className="dap-detail-row">
      <span className="dap-detail-label">{label}</span>
      {node ? <div className="dap-detail-val">{node}</div> : <span className="dap-detail-val">{value || "—"}</span>}
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: "72px 16px 16px" }}>
      ...
      <div className="dap-modal" style={{ position: "relative", width: "100%", maxWidth: 820, maxHeight: "calc(100vh - 90px)", overflowY: "auto", background: C.surface, borderRadius: 24, boxShadow: `0 40px 100px rgba(107,33,168,.22)` }}>
        <Tri h={4} />

        {/* Header modal */}
        <div style={{ padding: "24px 28px 0", background: `linear-gradient(180deg,${C.dapPale},${C.surface})`, borderBottom: `1px solid ${C.divider}` }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              <div style={{ width: 54, height: 54, borderRadius: 15, flexShrink: 0, background: C.dapPale, border: `1.5px solid ${C.dap}28`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 16px ${C.dap}18` }}>
                <GraduationCap size={23} color={C.dap} />
              </div>
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 10px", borderRadius: 6, background: C.dapPale, border: `1px solid ${C.dap}20`, marginBottom: 6 }}>
                  <BookOpen size={10} color={C.dap} />
                  <span style={{ fontSize: 10, fontWeight: 800, color: C.dap, textTransform: "uppercase", letterSpacing: ".1em" }}>DAP — Direction Apprentissage Professionnel</span>
                </div>
                <h2 className="dap-serif" style={{ fontSize: 21, fontWeight: 700, color: C.textPri, letterSpacing: "-.4px", lineHeight: 1.2 }}>
                ...
                <div style={{ display: "flex", gap: 7, alignItems: "center", marginTop: 8, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 11, color: C.dap, background: `${C.dap}10`, padding: "3px 9px", borderRadius: 6, border: `1px solid ${C.dap}20`, fontWeight: 800, fontFamily: "monospace" }}>
                    {dapId}
                  </span>
                  {formation.antenne && (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
            {[
              ...
              { label: "Hommes",     value: nbH,                                               icon: User,      color: C.dap   },
              ...
              return (
                <div key={i} style={{ background: `${k.color}08`, borderRadius: 12, padding: "10px 12px", border: `1px solid ${k.color}20`, textAlign: "center" }}>
                  ...
                  <p className="dap-serif" style={{ fontSize: 20, fontWeight: 700, color: k.color, lineHeight: 1 }}>{k.value}</p>
          ...
          <div style={{ display: "flex", gap: 2 }}>
            {tabs.map((tab) => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="dap-tab-btn"
                  style={{ color: active ? C.textPri : C.textMuted, background: active ? C.surface : "transparent", borderBottom: active ? `2px solid ${C.dap}` : "2px solid transparent" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ background: C.surfaceEl, borderRadius: 14, padding: "4px 18px", border: `1px solid ${C.divider}` }}>
                <div style={{ padding: "10px 0 6px", borderBottom: `1px solid ${C.divider}` }}>
                  <span style={{ fontSize: 10.5, fontWeight: 800, color: C.dap, textTransform: "uppercase", letterSpacing: ".08em" }}>Formation</span>
                </div>
              ...
              <div style={{ background: C.surfaceEl, borderRadius: 14, padding: "4px 18px", border: `1px solid ${C.divider}` }}>
                ...
                <InfoRow label="Total apprenants" node={
                  <span className="dap-serif" style={{ fontSize: 20, fontWeight: 700, color: C.green }}>{candidats.length || formation.nb_candidats || 0}</span>
                } />
          ...
          {activeTab === "apprenants" && (
            loading ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "52px 0", gap: 14 }}>
                <Loader2 size={26} color={C.dap} className="dap-spin" />
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "52px 0", gap: 12 }}>
                ...
                <p className="dap-serif" style={{ fontSize: 15, fontWeight: 600, color: C.textSub }}>Aucun apprenant rattaché</p>
              ...
              <div>
                <div style={{ position: "relative", marginBottom: 14 }}>
                  ...
                  <input className="dap-input" placeholder="Nom, identifiant, contact…" value={searchCand}
                ...
                <div style={{ borderRadius: 14, border: `1px solid ${C.divider}`, overflow: "hidden" }}>
                  <div style={{ padding: "11px 16px", background: `${C.navy}04`, borderBottom: `1px solid ${C.divider}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <span style={{ fontSize: 11, color: C.dap, background: `${C.dap}10`, padding: "2px 8px", borderRadius: 5, fontWeight: 700 }}>{nbH}H</span>
                      ...
                      {filteredCands.map((c, i) => (
                        <tr key={c.id} style={{ borderBottom: i < filteredCands.length - 1 ? `1px solid ${C.divider}` : "none", background: i % 2 === 0 ? C.surface : `${C.navy}008` }}>
                          ...
                          <td style={{ padding: "10px 14px" }}>
                            ...
                            {(c.sexe || c.genre) && (
                              <span style={{ fontSize: 10, color: (c.sexe || c.genre) === "F" ? C.teal : C.dap, fontWeight: 700 }}>
                          ...
                          <td style={{ padding: "10px 14px" }}>
                            {c.identifiant_unique
                              ? <span style={{ fontSize: 11, color: C.dap, background: `${C.dap}10`, padding: "3px 8px", borderRadius: 5, fontWeight: 800, fontFamily: "monospace", border: `1px solid ${C.dap}20` }}>{c.identifiant_unique}</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {loading ? (
                <div style={{ display: "flex", justifyContent: "center", padding: "40px 0" }}>
                  <Loader2 size={22} color={C.dap} className="dap-spin" />
                </div>
              ) : (
                <>
                  <div style={{ background: C.surfaceEl, borderRadius: 14, padding: "18px 20px", border: `1px solid ${C.divider}` }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
                      {[
                        { label: "Hommes", value: nbH, color: C.dap,  pct: candidats.length > 0 ? Math.round((nbH / candidats.length) * 100) : 0 },
                        ...
                        <div key={g.label} style={{ background: `${g.color}08`, borderRadius: 12, padding: "14px 16px", border: `1px solid ${g.color}20` }}>
                          <p className="dap-serif" style={{ fontSize: 28, fontWeight: 700, color: g.color, lineHeight: 1 }}>{g.value}</p>
                          ...
                          <div style={{ height: 5, borderRadius: 3, background: `${g.color}15`, overflow: "hidden", marginTop: 10 }}>
                            <div className="dap-bar" style={{ "--bw": `${g.pct}%`, height: "100%", borderRadius: 3, background: g.color }} />
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      return (
                        <div key={key} style={{ marginBottom: 10 }}>
                          ...
                          <div style={{ height: 7, borderRadius: 4, background: C.surfaceEl, overflow: "hidden", border: `1px solid ${C.divider}` }}>
                            <div className="dap-bar" style={{ "--bw": `${pct}%`, height: "100%", borderRadius: 4, background: cfg.c }} />
                          </div>
                        </div>
                      );
                    })}
                    ...
                    <div style={{ background: C.surfaceEl, borderRadius: 14, padding: "18px 20px", border: `1px solid ${C.divider}` }}>
                      ...
                      {Object.entries(
                        ...
                        <div key={domaine} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.divider}` }}>
                          ...
                          <span style={{ fontSize: 12, fontWeight: 700, color: C.dap, background: `${C.dap}10`, padding: "2px 8px", borderRadius: 5 }}>{count}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div style={{ padding: "14px 28px 20px", borderTop: `1px solid ${C.divider}`, display: "flex", justifyContent: "flex-end" }}>
          <button className="dap-btn-sec" onClick={onClose}><X size={13} /> Fermer</button>
        </div>
      </div>
    </div>
  );
...
/* ═══════════════════════════════════════════════════════════════════
   CARTE GRILLE DAP
═══════════════════════════════════════════════════════════════════ */
const DAPCard = ({ f, dapId, onClick, delay = 0 }) => {
  const ss    = getSessionStatus(f.date_debut, f.date_fin);
  const SSI   = ss.icon;
  const duree = dureeJours(f.date_debut, f.date_fin);
  return (
    <div className={`dap-card dap-in dap-d${delay}`} onClick={onClick}>
      <div style={{ height: 3, background: `linear-gradient(90deg, ${C.dap}, ${C.dapLight})` }} />
      <div style={{ position: "absolute", bottom: -28, right: -28, width: 120, height: 120, borderRadius: "50%", background: `${C.dap}05`, pointerEvents: "none" }} />
      <div style={{ padding: "18px 20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: C.dapPale, border: `1.5px solid ${C.dap}22`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <GraduationCap size={18} color={C.dap} />
            </div>
            <span className="dap-badge" style={{ background: C.dapPale, color: C.dap, border: `1px solid ${C.dap}30` }}>DAP</span>
          </div>
          <span className="dap-badge" style={{ background: ss.bg, color: ss.c, border: `1px solid ${ss.c}25` }}>
            <SSI size={9} /> {ss.l}
          </span>
        </div>
        <h3 className="dap-serif" style={{ fontSize: 15, fontWeight: 700, color: C.textPri, lineHeight: 1.3, marginBottom: 4, letterSpacing: "-.2px" }}>{f.nom_formation}</h3>
        <p style={{ fontSize: 11.5, color: C.textMuted, marginBottom: 11 }}>{f.organisme_formation || "—"}</p>
        <span style={{ fontSize: 10, color: C.dap, background: `${C.dap}10`, padding: "2px 8px", borderRadius: 5, fontWeight: 800, fontFamily: "monospace", border: `1px solid ${C.dap}20` }}>{dapId}</span>
        ...
        <div style={{ marginTop: 12, paddingTop: 11, borderTop: `1px solid ${C.divider}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <span className="dap-serif" style={{ fontSize: 17, fontWeight: 700, color: C.textPri }}>{f.nb_candidats || 0}</span>
            <span style={{ fontSize: 11, color: C.textMuted }}>apprenant{f.nb_candidats > 1 ? "s" : ""}</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: C.dap, fontWeight: 700 }}>Voir <ArrowUpRight size={11} /></div>
        </div>
      </div>
    </div>
  );
...
/* ═══════════════════════════════════════════════════════════════════
   COMPOSANT PRINCIPAL — FormationDAPc
═══════════════════════════════════════════════════════════════════ */
const FormationDAPc = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // Filtrer uniquement les formations DAP (type_formation === "apprentissage")
      setFormations(all.filter((f) => f.type_formation === "apprentissage"));
    } catch {}
    finally { setLoading(false); }
  }, []);
  ...
  const handleExportExcel = () => {
    setExporting(true);
    setTimeout(() => {
      try { exportExcel(formations, sorted); }
  ...
  const handleExportPDF = () => {
    exportPDF(formations, sorted);
  };

  /* ── Colonne triable ── */
  const SortHeader = ({ col, label, hide = false }) => (
    <th onClick={() => handleSort(col)}
      className={hide ? "dap-hide-sm" : ""}
      style={{ padding: "13px 16px", textAlign: "left", fontSize: 10, fontWeight: 800, color: sortCol === col ? C.dap : C.textMuted, letterSpacing: ".12em", textTransform: "uppercase", whiteSpace: "nowrap", cursor: "pointer", userSelect: "none" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        {label}
        {sortCol === col && <span style={{ color: C.dap, fontSize: 12 }}>{sortDir === "asc" ? "↑" : "↓"}</span>}
      </div>
    </th>
  );
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="dap dap-page" style={{ minHeight: "100vh", background: `radial-gradient(ellipse 100% 50% at 60% -8%, rgba(107,33,168,.07) 0%, transparent 60%), ${C.page}`, padding: "88px 28px 70px", position: "relative" }}>

        {/* Aurora bg */}
        <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden" }}>
          <div className="dap-aurora" style={{ position: "absolute", top: "-10%", right: "8%", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle,rgba(107,33,168,.07) 0%,transparent 70%)", filter: "blur(30px)" }} />
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1260, margin: "0 auto" }}>
          ...
          {/* ── En-tête ── */}
          <div className="dap-in dap-d0" style={{ marginBottom: 28 }}>
            <div style={{ width: 80, marginBottom: 14 }}><Tri h={3} /></div>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 12px", borderRadius: 20, background: C.dapPale, border: `1px solid ${C.dap}20`, marginBottom: 10 }}>
                  <BookOpen size={12} color={C.dap} />
                  <span style={{ fontSize: 11, fontWeight: 800, color: C.dap, textTransform: "uppercase", letterSpacing: ".12em" }}>DAP — Direction Apprentissage Professionnel</span>
                </div>
                <h1 className="dap-serif" style={{ fontSize: 34, fontWeight: 700, color: C.textPri, letterSpacing: "-.8px", lineHeight: 1.05 }}>
                  Formations Apprentissage
                </h1>
                <p style={{ fontSize: 13.5, color: C.textMuted, marginTop: 8 }}>
                  Liste des formations de la Direction Apprentissage Professionnel — ONFPP Guinée
                </p>
              </div>
              <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
                {/* Boutons export */}
                <button
                  className="dap-btn-export dap-btn-excel"
                  onClick={handleExportExcel}
                  disabled={exporting || sorted.length === 0}
                  style={{ opacity: sorted.length === 0 ? .5 : 1 }}>
                  <FileSpreadsheet size={13} />
                  {exporting ? "Export…" : "Excel"}
                </button>
                <button
                  className="dap-btn-export dap-btn-pdf"
                ...
                <button className="dap-btn-sec" onClick={fetchAll}>
                  <RefreshCw size={13} />
                </button>
              </div>
            </div>
          ...
          {/* ── KPIs ── */}
          <div className="dap-in dap-d1" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px,1fr))", gap: 13, marginBottom: 26 }}>
            {[
              { label: "Total formations", value: stats.total,      color: C.dap,   bg: C.dapPale,   icon: GraduationCap, bar: 1 },
              ...
              { label: "En cours",         value: stats.en_cours,   color: C.dap,   bg: C.dapSoft,   icon: TrendingUp,    bar: stats.en_cours   / Math.max(stats.total, 1) },
              ...
              return (
                <div key={i} style={{ background: C.surface, borderRadius: 17, padding: "17px 15px", border: `1px solid ${C.divider}`, boxShadow: `0 2px 14px ${C.shadow}`, position: "relative", overflow: "hidden" }}>
                  ...
                  <p className="dap-serif" style={{ fontSize: 28, fontWeight: 700, color: C.textPri, lineHeight: 1, letterSpacing: "-1px" }}>{s.value}</p>
                  ...
                  <div style={{ height: 4, borderRadius: 3, background: C.surfaceEl, overflow: "hidden" }}>
                    <div className="dap-bar" style={{ "--bw": `${Math.round(s.bar * 100)}%`, height: "100%", background: `linear-gradient(90deg,${s.color},${s.color}77)`, borderRadius: 3, animationDelay: `${i * .1 + .3}s` }} />
                  </div>
                </div>
              );
            })}
          ...
          {/* ── Filtres ── */}
          <div className="dap-in dap-d2" style={{ background: C.surface, border: `1px solid ${C.divider}`, borderRadius: 16, padding: "13px 18px", marginBottom: 18, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", boxShadow: `0 2px 14px ${C.shadow}` }}>
            <div style={{ position: "relative", flex: "1 1 230px", minWidth: 0 }}>
              ...
              <input className="dap-input" placeholder="Formation, formateur, organisme, entreprise…" value={search}
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              {[
                ...
                { v: "en_cours",  l: "En cours",    c: C.dap     },
                { v: "terminee",  l: "Terminées",   c: C.green   },
              ].map((f) => (
                <button key={f.v} className="dap-pill" onClick={() => { setFilterStat(f.v); setPage(1); }}
            ...
            <select className="dap-input" value={filterAnt} onChange={(e) => { setFilterAnt(e.target.value); setPage(1); }}
          ...
          {/* ── Contenu principal ── */}
          <div className="dap-in dap-d3" style={{ background: C.surface, border: `1px solid ${C.divider}`, borderRadius: 20, boxShadow: `0 2px 18px ${C.shadow}`, overflow: "hidden" }}>

            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "72px 0", gap: 16 }}>
                <Loader2 size={30} color={C.dap} className="dap-spin" />
                <p style={{ fontSize: 13.5, color: C.textMuted }}>Chargement des formations DAP…</p>
              </div>

            ) : sorted.length === 0 ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "72px 0", gap: 14 }}>
                <div style={{ width: 62, height: 62, borderRadius: 17, background: C.dapPale, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${C.dap}20` }}>
                  <GraduationCap size={27} color={C.dap} />
                </div>
                <p className="dap-serif" style={{ fontSize: 16, fontWeight: 600, color: C.textSub }}>Aucune formation DAP trouvée</p>
                <p style={{ fontSize: 13, color: C.textMuted }}>
                  {search || filterAnt || filterStat !== "tous" ? "Modifiez vos critères de recherche" : "Les formations DAP créées via Inscriptions apparaîtront ici"}
                </p>
              </div>
              ...
              <div style={{ padding: "20px" }}>
                <div className="dap-grid">
                  {paged.map((f, i) => (
                    <DAPCard key={f.id} f={f} dapId={buildDapId(f, gIdx(f))}
                      onClick={() => setDetail({ formation: f, id: buildDapId(f, gIdx(f)) })}
                      delay={Math.min(i, 4)} />
                  ))}
                </div>
              </div>
              ...
              <>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: `linear-gradient(90deg, ${C.dap}08, transparent)`, borderBottom: `1.5px solid ${C.divider}` }}>
                        ...
                        <th className="dap-hide-sm" style={{ padding: "13px 16px", textAlign: "left", fontSize: 10, fontWeight: 800, color: C.textMuted, letterSpacing: ".12em", textTransform: "uppercase" }}>Entreprise</th>
                        <SortHeader col="date_debut"          label="Période"      />
                        <th className="dap-hide-sm" style={{ padding: "13px 16px", textAlign: "left", fontSize: 10, fontWeight: 800, color: C.textMuted, letterSpacing: ".12em", textTransform: "uppercase" }}>Durée</th>
                    ...
                    <tbody>
                      {paged.map((f, ri) => {
                        ...
                        const dur = dureeJours(f.date_debut, f.date_fin);
                        return (
                          <tr key={f.id} className="dap-tr"
                            style={{ borderBottom: `1px solid ${C.divider}`, background: ri % 2 === 0 ? C.surface : `${C.navy}008` }}
                            onClick={() => setDetail({ formation: f, id: fid })}>

                            {/* ID */}
                            <td style={{ padding: "13px 16px" }}>
                              <span style={{ fontSize: 10.5, fontWeight: 800, color: C.dap, background: `${C.dap}10`, padding: "4px 10px", borderRadius: 6, fontFamily: "monospace", border: `1px solid ${C.dap}22`, whiteSpace: "nowrap" }}>
                                {fid}
                              </span>
                            </td>
                            ...
                            <td style={{ padding: "13px 16px" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                                <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: C.dapPale, border: `1px solid ${C.dap}20`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                                  <GraduationCap size={15} color={C.dap} />
                                </div>
                                <div>
                            ...
                            {/* Entreprise */}
                            <td className="dap-hide-sm" style={{ padding: "13px 16px" }}>
                              {f.entreprise_formation ? (
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            <td style={{ padding: "13px 16px" }} onClick={(e) => { e.stopPropagation(); setDetail({ formation: f, id: fid }); }}>
                              <div className="dap-act-btn"
                                style={{ background: `${C.dap}08`, borderColor: `${C.dap}20` }}
                                onMouseEnter={(e) => { e.currentTarget.style.background = `${C.dap}18`; e.currentTarget.style.borderColor = C.dap; }}
                                onMouseLeave={(e) => { e.currentTarget.style.background = `${C.dap}08`; e.currentTarget.style.borderColor = `${C.dap}20`; }}>
                                <Eye size={13} color={C.dap} />
                              </div>
                            </td>
                          </tr>
                        );
                ...
                <div style={{ padding: "10px 20px", background: `${C.navy}02`, borderTop: `1px solid ${C.divider}`, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ height: 12, display: "flex", borderRadius: 2, overflow: "hidden", width: 28, flexShrink: 0 }}>
                  ...
                  {[
                    ...
                    { c: C.dap,   l: "En cours",   icon: TrendingUp   },
                  ...
                  <span style={{ fontSize: 11, color: C.textMuted, marginLeft: "auto" }}>
                    Cliquez sur ↑↓ les en-têtes pour trier · Cliquez sur une ligne pour voir le détail
                  </span>
                </div>
              </>
            )}
            ...
            {!loading && sorted.length > 0 && pages > 1 && (
              <div style={{ padding: "13px 20px", borderTop: `1px solid ${C.divider}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                  <button className="dap-pg" onClick={() => setPage((p) => p - 1)} disabled={page === 1}><ChevronLeft size={13} /></button>
                  {Array.from({ length: pages }, (_, i) => i + 1)
                    .filter((p) => p === 1 || p === pages || Math.abs(p - page) <= 1)
                    .map((p, i, arr) => (
                      <React.Fragment key={p}>
                        {i > 0 && arr[i - 1] !== p - 1 && <span style={{ color: C.textMuted, fontSize: 12, padding: "0 2px" }}>…</span>}
                        <button className={`dap-pg${p === page ? " active" : ""}`} onClick={() => setPage(p)}>{p}</button>
                      </React.Fragment>
                    ))}
                  <button className="dap-pg" onClick={() => setPage((p) => p + 1)} disabled={page === pages}><ChevronRight size={13} /></button>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Note bas de page */}
          <div className="dap-in dap-d4" style={{ marginTop: 14, padding: "12px 18px", background: `${C.navy}03`, borderRadius: 12, border: `1px solid ${C.divider}`, display: "flex", alignItems: "center", gap: 10 }}>
            <BookOpen size={13} color={C.textMuted} />
            <p style={{ fontSize: 12, color: C.textMuted, lineHeight: 1.5 }}>
              Affiche uniquement les formations de type <strong style={{ color: C.dap }}>Apprentissage (DAP)</strong>.
              Utilisez les boutons <strong style={{ color: "#166534" }}>Excel</strong> et <strong style={{ color: "#9F1239" }}>PDF</strong> pour exporter la liste filtrée.
